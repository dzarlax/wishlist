...
Status transitions: `available` → `reserved` → `purchased` → `gifted` (archive)
- Owner can mark any non-gifted gift as `gifted` (admin-only action)
- `gifted` gifts are moved to a collapsed archive section
