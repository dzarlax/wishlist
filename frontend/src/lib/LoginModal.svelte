    ...
    <div class="p-7 space-y-5">
      {#if ssoConfig.sso}
        ...
        <button
          ...
          class="w-full py-3 px-4 rounded-none {designSystem.text.weight
            .semibold} text-white bg-indigo-600 hover:bg-indigo-500 transition-all duration-300"
        >
          {$t('auth.loginWithSso')}
        </button>
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="{designSystem.text.xs} text-black/40 dark:text-white/40"
            >{$t('auth.or')}</span
          >
          <div class="flex-1 h-px bg-black/10 dark:bg-white/10"></div>
        </div>
    ...
    >
      <button
        ...
        class="px-4 py-2 rounded-none {designSystem.text.weight
          .semibold} text-graphite dark:text-dark-text hover:text-black dark:hover:text-white {designSystem
          .color.neutral.background.surface} {designSystem.color.neutral.background
          .surfaceDark} hover:bg-[#d8d6d3] dark:hover:bg-[#25292d] transition-all duration-300 disabled:opacity-50"
      >
        {$t('actions.cancel')}
      </button>
      <button
